...
def part1(input_list: list[str]) -> int:
    ...
    for row, line in enumerate(input_list):
        # Get column for each '#' in the line
    ...
    # Shift the locations by X*1 times of expanded places before them
    galaxy_locations = [(row + sum(row > x for x in to_expand_rows),
                         col + sum(col > y for y in to_expand_cols))
...
if __name__ == "__main__":
    print(part1(file_contents))  # 10231178
