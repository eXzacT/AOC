...
def pos_after_falling(brick_id: int, start: tuple[int, int, int], end: tuple[int, int, int]) -> dict[int, list[tuple[int, int, int]]]:
    ...
    for pos in positions:
        ...
        # Shouldn't this be the same?pos[2]
...
def possible_bricks_to_remove(bricks: dict[int, list[tuple[int, int, int]]]):
...
def part1(data: list[int]) -> int:
    ...
    return possible_bricks_to_remove(bricks)


if __name__ == "__main__":
    print(part1(data))  # 375
